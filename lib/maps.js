...
maps.buildInfoWindow = function (propContent) {
  ...
  maps._map.controls[google.maps.ControlPosition.BOTTOM_LEFT].clear();
  maps._map.controls[google.maps.ControlPosition.BOTTOM_LEFT].push(document.getElementById('infoWindow'));
  maps._map.addListener('mousemove', function (event) {
    if (event.feature == undefined) maps._map.controls[google.maps.ControlPosition.BOTTOM_LEFT].clear();
  })
}
    ...
    for (var key in propertyValue) {
      if (propertyValue.hasOwnProperty(key)) {
        propertyValue[key].forEach(function (value) {
          if (matches || f.getProperty(key) === value) {
            matches = true;
          }
        });
      }
...
maps.groupAreasByProperty = function(property) {
  if (property != undefined && property != '' && !document.getElementById(maps._legendDivId)) {
    maps.buildLegend();
  }
...
maps.EditAreaBtn = function (controlDiv, event, type) {
  ...
  if (type == 'done') controlText.innerHTML = 'Done';
  if (type == 'cancel') controlText.innerHTML = 'Cancel';
...
maps.editArea = function(event, buttonId, infowindow) {
  ...
  button.addEventListener("click", function() {
    ...
    var doneBtn = new maps.EditAreaBtn(centerControlDiv, event, 'done');
    var cancelBtn = new maps.EditAreaBtn(centerControlDiv, event, 'cancel');
  ...
  maps._map.data.addListener(ev, function (event) {
    if (!maps._editInPrgrs) {
      ...
      if(toolTipOnHover) {
        ...
        if (editable) {
          maps.editArea(event.feature, 'editArea', infowindow);
        }
      }
    }
  });
...
maps.getScreenshot = function (feature, size, format) {
  size = size || "400x400";
  format = format || "png";
  var paths = "color:0x" + maps._usedColors[feature.getProperty(propertyToGroupBy)].replace("#", "") +
    "|fillcolor:0x" + maps._usedColors[feature.getProperty(propertyToGroupBy)].replace("#", "") + "|weight:2|enc:";
  var latlngs = [];
