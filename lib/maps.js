...
maps.Maps = function (apiKey, mapDivId) {
  ...
  this.createNewArea = function () {
    ...
    var low = -1;
    if (mapThis._allAreas.hasOwnProperty('features')) {
      mapThis._allAreas['features'].forEach(function (area) {
        if (area['properties'].hasOwnProperty('id') && area['properties']['id'] > low) low = area['properties']['id'];
      });
    }
    this._map.data.addListener('addfeature', function (event) {
      event.feature.setProperty('id', low + 1);
      mapThis._map.data.setDrawingMode(null);
      google.maps.event.clearListeners(mapThis._map, 'addfeature');
      mapThis.saveData();
    });
  };
     ...
     * @param id - give id to the line
     */

  this.createLine = function (id) {
    var mapThis = this;
    ...
    drawingManager.setMap(mapThis._map);
    google.maps.event.addListener(drawingManager, 'polylinecomplete', function(event) {
      drawingManager.setDrawingMode(null);
      drawingManager.setOptions({drawingControl: false});
      mapThis._allLines[id] = event;
    });
  };

    /**
     * Loads the lines from _allLines variable.
     */

  this.loadLines = function () {
    var mapThis = this;
      for (line in mapThis._allLines) {
          mapThis._allLines[line].setMap(mapThis._map);
      }
  };

    /**
     * Deletes the line with given id.
     * @param id - give id to the line
     */
  this.deleteLine = function (id) {
    var mapThis = this;
    if(mapThis._allLines[id] != null) {
      var line = mapThis._allLines[id];
      line.setMap(null);
        mapThis._allLines[id] = null;
    }
  };
  ...
  this._EditAreaBtn = function (controlDiv, event, type, btnTxt, parent) {
    ...
    controlUI.addEventListener('click', function () {
      ...
      if (type == 'cancel') event.setGeometry(oldgeo);
   ...
   * @param id - Feature that will be edited
   ...
   */
  this.editArea = function (id, editDoneTxt, editCancelTxt) {
    var mapThis = this;
    editDoneTxt = editDoneTxt || "Done";
    editCancelTxt = editCancelTxt || "Cancel";
    var feature = {};
    mapThis._map.data.forEach(function (feat) {
      var propertyId = feat.getProperty("id");
      if (propertyId == id) {
        feature = feat;
      }
    });
  ...
  this._editArea = function (feature, buttonId, infowindow, editDoneTxt, editCancelTxt) {
    var mapThis = this;
    var button = document.getElementById(buttonId);
    ...
    this._map.data.addListener(ev, function (event) {
      if (!mapThis._editInPrgrs) {
        ...
        if (toolTipOnHover) {
          ...
          if (editable) {
            content += "<p>" + "<button id='_editArea'>" + editBtnTxt + " </button>" + "</p>";
          }
          ...
          if (editable) {
            mapThis._editArea(event.feature, '_editArea', infowindow, editDoneTxt, editCancelTxt);
          }
        }
      }
    });
