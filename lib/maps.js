 ...
 * @param propValue
 */
maps.listenForAreaSelections = function (propValue) {
  maps._map.data.addListener("click", function (event) {
    for (var key in propValue) {
      if (propValue.hasOwnProperty(key)) {
        if (utf8fy(event.feature.getProperty(key)) == propValue[key] &&
          maps.areasSelected.indexOf(event.feature) == -1) maps.areasSelected.push(event.feature);
      }
    }
  })
}
 ...
 * @param googleMaps
 */
maps.getInfoWindowforArea = function() {
    var ev;
    if (toolTipOnHover) {
        ev = 'mouseover';
    } else {
    ...
    maps._map.data.addListener(ev, function (event) {
      ...
      infowindow.setContent(
        "<b>Kultuur:</b> " + utf8fy(event.feature.getProperty('kultuur')) + "<br/>" +
        "<b>Pindala: </b>" + event.feature.getProperty('pindala') + " ha" + "<br/>" +
        "<p>" + "<button id='editArea'>" + "CLICK ME TO EDIT FIELD" + " </button>" + "</p>"
      );
