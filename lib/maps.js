...
maps._legendDivId = "";
maps._cb = false;
maps._btnNameCasualColors = "Värvipimedatele";
maps._btnNameCBColors = "Tavavärvid";
maps._legendObj = {};
maps._colors_cb = [
  '#d73027', //cb
  '#313695',  //cb
...
maps.buildLegend = function (toggableButtonText) {
  ...
  if(colToggable) {
    ...
    button.innerHTML = '<button id="colortoggableID" class="btn btn-success btn-square btn-block">' + toggableButtonText +  '</button>';
...
maps.toggleColorblind = function (buttonId) {
  ...
  button.addEventListener("click", function() {
    ...
    if(maps._cb) {
      maps.buildLegend(maps._btnNameCBColors);
    } else {
      maps.buildLegend(maps._btnNameCasualColors);
    }
    maps.loadData();
  });
}
