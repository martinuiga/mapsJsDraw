...
maps.buildLegend = function (colToggable, toggableButtonText) {
  colToggable = colToggable || true;
...
maps.updateGeometry = function(id, feature) {
  var replaced = false;
  maps._map.data.forEach(function (featurem) {
    if (featurem.getProperty("id") == id) {
      var newLinRingArr = [];
      ...
      featurem.setGeometry(newMultiPoly);
      replaced = true;
    }
  });
...
maps.appendLegendContent = function (col, propContent, legendObj) {
...
maps.groupAreasByProperty = function(property) {
  ...
  maps._map.data.forEach(function (feature) {
    ...
    if (!(propContent in maps._usedColors) && propContent != 'undefined') {
      ...
      if (property != undefined && property != '') maps.appendLegendContent(col, propContent, legendObj);
...
maps.zoomIntoArea = function (areaID) {
  ...
  maps._map.data.forEach(function(feature) {
    ...
    if (areaID.includes(propertyId)) {
      maps.processPoints(feature.getGeometry(), bounds.extend, bounds);
      maps._map.fitBounds(bounds);
    }
  });
}
