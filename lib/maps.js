...
maps.Maps = function (apiKey, mapDivId) {
  ...
  this.buildInfoWindow = function (propContent) {
    ...
    this._map.controls[google.maps.ControlPosition.LEFT_BOTTOM].push(document.getElementById('infoWindow'));
   ...
   */
  this.addMarker = function (lat, lng, info) {
    var mapThis = this;
   ...
   * @param selected
   */
  this.deleteAreas = function (id, selected) {
    console.log(this);
    var mapThis = this;
  ...
  this.listenForAreaSelections = function (callback) {
    ...
    this._map.data.addListener("click", function (event) {
      ...
      var index = -1;
      var selected = true;
      if ((index = mapThis._selectedAreas.indexOf(id)) == -1) {
        mapThis._selectArea(id, feature);
      } else {
        mapThis._deselectArea(id, feature);
        selected = false
      }
      callback.call(mapThis, id, selected);
    })
  };
  ...
  this._EditAreaBtn = function (controlDiv, event, type, btnTxt) {
    var mapThis = this;
  ...
  this.editArea = function (feature, buttonId, infowindow, editDoneTxt, editCancelTxt) {
    ...
    button.addEventListener("click", function () {
      ...
      var doneBtn = new mapThis._EditAreaBtn(centerControlDiv, feature, 'done', editDoneTxt);
      var cancelBtn = new mapThis._EditAreaBtn(centerControlDiv, feature, 'cancel', editCancelTxt);
