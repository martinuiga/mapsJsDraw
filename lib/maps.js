...
function editArea(googlemaps, event) {
    //googlemaps.data.overrideStyle(event, {editable: true});
    console.log(event);
}
...
function getInfoWindowforArea(googleMaps) {
    ...
    googleMaps.data.addListener('click', function (event) {
        infowindow.setContent("<b>Kultuur:</b> " + utf8fy(event.feature.getProperty('kultuur')) + "<br/>" + "<b>Pindala: </b>" +
            ...
            "<button id='editArea' onclick='editArea(googleMaps, event.feature)'>" + "CLICK ME TO EDIT FIELD" + " </button>" + "</p>");
